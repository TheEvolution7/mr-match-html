...
jQuery(".hide-show-shortcut").click(function(){
	//console.log(shortcut);
	if ($('.keyboard-shortcut').hasClass('hide'))
	{
		jQuery('.keyboard-shortcut').removeClass('hide');
		jQuery(this).text('Hide')
	}
	else
	{
		jQuery('.keyboard-shortcut').addClass('hide');
		jQuery(this).text('Show');
	}
});
if ($('emoji-picker').length>0){
$('#emoji-picker-trigger').click(function(){
	if ($('#emoji-picker-trigger').hasClass('active'))
		{
			$('emoji-picker').removeClass('active');
	$('#emoji-picker-trigger').removeClass('active');
		}
		else
		{
	$('emoji-picker').addClass('active');
	$('#emoji-picker-trigger').addClass('active');
		}
});

document.querySelector('emoji-picker')
  .addEventListener('emoji-click', event => $('#chatarea').val( $('#chatarea').val()+event.detail.unicode));
}
if ($('#sticker').length>0){
	$('#sticker').click(function (){
		$('#sticker-holder').css('display', 'flex');
		$('#sticker-holder .spinner-border').show();
		var html='';
		$.get('https://api.giphy.com/v1/stickers/trending?api_key=pQR3FrDiZz9ZudlbHeB2hQMIM0Rt3MKL&limit=10&offset=0&rating=g&bundle=messaging_non_clips', function(response) {
			$.each( response.data, function( key, value ) {
				$('#sticker-holder').append("<img src=" + response.data[key].images.fixed_width.url + ">");
...
$('#messages-tab-pane>ul>li').click(function(){
